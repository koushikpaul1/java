package com.edge.collection.D_list;
...
class ArrayListDemo {
	...
	/*
	 * Many are of the assumption that multiple insertion and removal operations on
	 * ArrayList will decrease the performance of an application. But, there will be
	 * no significant change in the performance of an application if you use
	 * ArrayList instead of arrays. Below example shows time taken to add 1000
	 * string elements to ArrayList and array.
	 */
	void speedTest(){
		...
		for(int i = 0;i <= 100000;i++)
			nameList.add("Name" + i);
		// for (int i = 0; i <= 100000; i++)nameList.add(0,"Name"+i);
		endTime = System.currentTimeMillis();
		System.out.println("Time taken by ArrayList : " + (endTime - startTime) + "ms");
		// for(String s: namesArray) {System.out.println(s);}
...
//can traverse an ArrayList in both the directions – forward and backward using ListIterator.	
	void traverse(){
		...
		while (iterator.hasPrevious())
			System.out.print(iterator.previous() + " ");// doesn't print anything , because the pointer is at the
														// begining.
		while (iterator.hasNext())
			System.out.print(iterator.next() + " ");// ONE TWO THREE FOUR
		System.out.println("\nElements in backward direction");
		while (iterator.hasPrevious())
			System.out.print(iterator.previous() + " ");// FOUR THREE TWO ONE
	}
